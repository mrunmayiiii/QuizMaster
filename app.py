...
#     score=db.relationship('Score',back_populates='user',cascade='all,delete-orphan')
...
#     chap=db.relationship("Chapter", back_populates='sub',cascade="all,delete-orphan")
...
#     sub=db.relationship("Subject", back_populates='chap')
#     quiz=db.relationship("Quiz",back_populates='chapter',cascade='all,delete-orphan')
...
#     chapters_id=db.Column(db.Integer, db.ForeignKey("Chapter.id"),nullable=False)
#     chapter=db.relationship("Chapter", back_populates='quiz')
#     que=db.relationship("Questions",back_populates='quiz',cascade='all,delete-orphan')
#     score=db.relationship('Score',back_populates='quiz',cascade='all,delete-orphan')
...
#     quizes_id=db.Column(db.Integer, db.ForeignKey("Quiz.id"),nullable=False)
#     quiz=db.relationship("Quiz",back_populates='que')
...
#     quizes_id=db.Column(db.Integer, db.ForeignKey("Quiz.id"))
#     timestamp=db.Column(db.DateTime,nullable=False)
#     quiz=db.relationship('Quiz',back_populates='score')
...
#         admin=User(fullname="admin",email="admin@gmail.com",password=hashed_password,qualification="BS,iitm",dob="01-01-2000",is_admin=True)
...
#         user=User(fullname=fullname,email=email,password=hashed_password,qualification=qualification,dob=dob)
#         db.session.add(user)
#         db.session.commit()
...
#         print(subjects)
#         return render_template("admin.html",subjects=subjects,scores=scores)
#     return redirect("/login")
...
#     if 'admin' in session:
#         subject=Subject.query.filter(subid==subid).first()
#         chapter=Chapter.query.filter(subid==subid).all()
#         return render_template("showsubject.html",subject=subject,chapter=chapter)

# @app.route("/showquiz/<int:quizid>",methods=["POST", "GET"])
# def showquiz(quizid):
#     if 'admin' in session:
#         quiz=Quiz.query.filter(quizid==quizid).first()
#         questions=Questions.query.filter(quizid==quizid).all()
...
#         chapter = Chapter.query.filter_by(id=chapid).first()
#         quiz = Quiz.query.filter_by(chapters_id=chapid).all()

#         return render_template("showchapter.html",chapter=chapter,quiz=quiz)
...
#             newsub=Subject(name=name,description=description)
...
#     if 'admin' in session:
#         subject=Subject.query.filter_by(id=subid).first()
#         if not subject:
#             return redirect("/admin")
#         if request.method=="POST":
#             name=request.form["name"]
...
#             db.session.commit()
#             return redirect("/admin")
#         return render_template("delpopup.html",subject=subject)
#     return redirect("/login")
...
#             chapter=Chapter(name=name,description=description,subject_id=subid)
...
#             db.session.commit()
#             return redirect(url_for("showsubject",subid=subid))
#         return render_template("delchapterpopup.html",chapter=chapter)
#     return redirect("/login")
...
#             quiz=Quiz(date_ofquiz=dateofquiz,time_dur=time_dur,remarks=remarks,chapters_id=chapter_id)
#             db.session.add(quiz)
#             db.session.commit()
...
#                 db.session.commit()
#                 return redirect(url_for("showchapter",chapid=quiz.chapters_id))
#             else:
...
#             quizes_id=quizid
#             que=Questions(statement_que=statement_que,option1=option1,option2=option2,option3=option3,option4=option4,correction=correction,quizes_id=quizid)
#             db.session.add(que)
#             db.session.commit()
...
# def queedit(queid): 
#     que=Questions.query.filter_by(id=queid).first()
#     if 'admin' in session:
#         if request.method=="POST":
#             statement_que=request.form["statement_que"]
#             option1=request.form["option1"]
...
#             db.session.commit()
#             return redirect(url_for("showquiz",quizid=que.quiz.id))
#         else:
...
#         return render_template('userdash.html',quizes=quizes,user=user)
#     return redirect('/login')

# #make a route to edit user profile details
...
#     if "user" in session:
#         questions=Questions.query.filter_by(quizes_id=quizid).all()
#         user=User.query.filter_by(id=session['user']).first()
#         score=0
#         print("Received form data:", request.form)  # Debugging
#         for q in questions:
#             user_answer = request.form.get(str(q.id), "")  # Safely get user input
#             correct_answer = q.correction.strip()  # Ensure no extra spaces
#             print(f"QID: {q.id} | User Answer: '{user_answer}' | Correct Answer: '{correct_answer}'")  # Debugging
#             if user_answer.strip().lower() == correct_answer.lower():  # Case insensitive comparison
#                 score += 1
#         print(score)
#         finalscore=Score(scores=score,user_id=user.id,quizes_id=quizid,timestamp=datetime.now())
#         db.session.add(finalscore)
#         db.session.commit()
#         return render_template("quizresult.html",score=score,quet=len(questions),user=user)
#     return redirect("/login")

# #creating user/history route to show all the previous attempted quizes to user
...
# #create route for admin search for user,subjects,chapters and quizes based on their names
...
#             if not search_query:  # If no search query, return an empty result set
#                 return render_template("adsearch.html", users=[], subjects=[], chapters=[], quizes=[])
#            # search_query = request.args.get('search', '')
#             print(f"Search Query: {search_query}")
#             users=User.query.filter(User.fullname.ilike('%'+ search_query+'%')).all()
#             subjects=Subject.query.filter(Subject.name.ilike('%'+ search_query+'%')).all()
#             chapters=Chapter.query.filter(Chapter.name.ilike('%'+ search_query+'%')).all()
#             quizes=Quiz.query.filter(Quiz.remarks.ilike('%'+ search_query+'%')).all()
...
#         # Count number of chapters per subject
#         chapcount = [(subject.name, Chapter.query.filter_by(subject_id=subject.id).count()) for subject in subjects]

#         # Count number of quizzes per chapter
#         quizcount = [(chapter.name, Quiz.query.filter_by(chapters_id=chapter.id).count()) for chapter in chapters]

#         # Count quizzes in each subject
#         quizsub = {subject.name: 0 for subject in subjects}
#         for subject in subjects:
#             subject_chapters = Chapter.query.filter_by(subject_id=subject.id).all()
#             for chapter in subject_chapters:
#                 quizsub[subject.name] += Quiz.query.filter_by(chapters_id=chapter.id).count()  
...
#         usertries = {user.fullname: Score.query.filter_by(user_id=user.id).count() for user in users}
...
def quizsubmission(quizid):
    if "user" in session:
        ...
        if "quiz_start_time" in session:
            ...
            # Parse the time duration correctly (format: "HH:MM" or "MM:SS")
            ...
            # If time limit exceeded, record a score of 0
